...
SLOP = re.compile(r"(in today's|rapidly evolving|game[- ]changer|\bunlock\w*|\bleverag\w*|seamless\w*|"
                  r"cutting[- ]edge|revolutioni\w+|synerg\w+|\bdelve\w*|hope this finds you|\b(ai|a\.i\.|genai|agentic)\s+governance\b|\bgovernance\s+(of|for)\s+(ai|agents)\b|\bresponsible\s+ai\b|\btrustworthy\s+ai\b|\bai\s+(risk|trust)\s+(platform|layer)\b|\u2014)", re.IGNORECASE)
