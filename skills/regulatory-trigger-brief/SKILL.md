...
description: Turn a regulatory trigger into a short, sourced account brief for an iTmethods motion, through the week1-wire MCP tools that enforce the audit rule, exclusions, routing limits and claim limits. Buyer-agnostic; the implemented trigger today is SR 26-2 for banks. Use when asked to brief an account on a regulatory trigger, run a motion's playbooks, or check whether an account should get a brief.
---

# Regulatory trigger to account brief

Buyer-agnostic: the steps below work for any playbook whose trigger is implemented. Bank and SR 26-2 details appear only as examples; the real specifics come from the tools.

You are drafting a brief that the named iTmethods account owner may forward to the account's senior risk or audit reader (for a bank, the chief audit executive). The reader is expert, busy and allergic to marketing. Every sentence must be true, sourced and specific to this account. You never contact anyone: the tools route the finished brief to the account owner, who decides.
...
1. `list_playbooks` - find the playbook for the buyer. Use only playbooks whose trigger is implemented (today, `bank-sr26-2`); for any other, stop and report the recorded reason. Everything buyer-specific comes from the playbook and the trigger record, not from these instructions.
2. `fetch_source` with the playbook's trigger id. Then **read the source URLs yourself** (for SR 26-2: the Federal Reserve letter, the OSFI E-23 and B-13 pages, the DORA page) and extract what changed in your own words. Cite only the source ids the tool returned.
...
4. Reason about this specific account before drafting:
   - Where is it headquartered, and which home-regulator rules are certain? Use `certain_context` from `check_applicability` and `context_by_jurisdiction` from `fetch_source`. Example: for a Canadian D-SIB (domestic systemically important bank), OSFI E-23 (model risk, including AI and machine learning models, effective May 1, 2027) and OSFI B-13 (technology and cyber risk) are certain.
   - Which of the trigger's `structural_conditions` are established on the record? Example for SR 26-2: a US banking entity supervised by the Federal Reserve, an EU financial entity. If not established, anything that depends on them is a "Confirm" line, never a statement.
...
8. Draft the brief in exactly this structure (headings verbatim):
   ...
   ```
   # <account name>: <trigger short name, e.g. SR 26-2> brief for the account owner
